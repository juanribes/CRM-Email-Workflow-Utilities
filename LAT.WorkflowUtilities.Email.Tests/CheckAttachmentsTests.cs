...
namespace LAT.WorkflowUtilities.Email.Tests
{
    ...
    public class CheckAttachmentsTests
    {
        ...
        public CheckAttachmentsTests()
        {
            ...
            _namespaceClassAssembly = "LAT.WorkflowUtilities.Email.CheckAttachments" + ", " + "LAT.WorkflowUtilities.Email";
        }
        #endregion
        ...
        public void ZeroAttachments()
        {
            ...
            {
                { "EmailToCheck", new EntityReference("email", Guid.NewGuid()) }
            };
            ...
            //Invoke the workflow
            var output = InvokeWorkflow(_namespaceClassAssembly, ref targetEntity, inputs, ZeroAttachmentsSetup);

            //Test(s)
            Assert.AreEqual(expectedAttachmentCount, output["AttachmentCount"]);
            Assert.AreEqual(expectedHasAttachments, output["HasAttachments"]);
        }
        ...
        private static Mock<IOrganizationService> ZeroAttachmentsSetup(Mock<IOrganizationService> serviceMock)
        {
            ...
            Entity emailCount = new Entity("email") { Id = Guid.Empty };
        ...
        public void OneAttachment()
        {
            ...
            //Invoke the workflow
            var output = InvokeWorkflow(_namespaceClassAssembly, ref targetEntity, inputs, OneAttachmentSetup);
        ...
        /// <param name="serviceMock">The Organization Service to mock</param>
        /// <returns>Configured Organization Service</returns>
        private static Mock<IOrganizationService> OneAttachmentSetup(Mock<IOrganizationService> serviceMock)
        {
            EntityCollection queryResult = new EntityCollection();

            Entity emailCount = new Entity("email") { Id = Guid.Empty };
            AliasedValue a = new AliasedValue("email", "activityid", 1);
            emailCount["count"] = a;
